...
sub xmlparser {
    my $self = shift;
    # if SOAP::Constants::DO_NOT_USE_XML_PARSER
    #   don't try XML::Parser
    # else 
    #   try XML::Parser
    #
    # or try XML::Parser::Lite
    # or die with appropriate error message
    return eval {
...
sub parser {
    my $self = shift;
    $self = $self->new() if not ref $self;    # make sure we have an object...
    @_
        ? ($self->{'_parser'} = shift, return $self)
...
sub final {
    ...
    # clean out last result on returning it
    return delete $self->{_done};
}


sub start {
    push @{ shift->{_values} }, [ shift, {@_} ];
}
...
sub is_xml {
    # Added check for envelope delivery. Fairly standard with MMDF and
    # sendmail
...
sub decode_object {
    ...
    my $ns = ($name =~ s/^($SOAP::Constants::NSMASK?)://)
        ? $1
        : '';
    ...
    $ref->[5] = SOAP::Utils::longname(
        ( $ns )
            ? ($uris{$ns} 
                || die "Unresolved prefix '$ns' for element '$name'\n")
            : (defined $uris{''})
                ? $uris{''} 
                : undef, 
        $name
...
sub decode_value {
    ...
    if (exists $attrs->{"{$SOAP::Constants::NS_ENC}arrayType"}) {
        ...
        my($type, $multisize) = 
            $attrs->{"{$SOAP::Constants::NS_ENC}arrayType"} =~ ~m{
                ^
                (.+)
                \[(\d*(?:,\d+)*)\]
                (?:\[(?:\d+(?:,\d+)*)\])*
        ...
        if (ref $children
            ...
            # TBD implement position in multidimensional array
                my($position) = SOAP::Utils::o_lattr($children->[$pos])
